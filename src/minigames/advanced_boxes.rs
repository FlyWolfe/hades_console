...
pub async fn run(mut dispenser: Option<TreatDispenser>) {
    ...
    let mut rng = rng();
    let mut current_location: Vec2 = Vec2::new(rng.random_range(0.0..=screen_width() - BOX_SIZE), rng.random_range(0.0..=screen_height() - BOX_SIZE));
    ...
    loop {
        ...
        if input::get_input_pos().is_some() {
            ...
            if check_box_touched(input, current_location, BOX_SIZE) {
                current_location = Vec2::new(rng.random_range(BOX_SIZE..=screen_width() - BOX_SIZE), rng.random_range(BOX_SIZE..=screen_height() - BOX_SIZE));
